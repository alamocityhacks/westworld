...
export default function Languages() {
    return (
        <div className="p-8">
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid-flow-row gap-12">
                {languages.map(({initial, language, featured}) => (
                    <Language initial={initial} language={language} featured={featured} />
                ))}
            </div>
            <p className="mt-2 text-sm font-inter">If you don't know any of these, then the <span className="text-yellow-500">gold</span> languages are languages you should learn. Choose <em>one</em> and learn it! CodeAcademy is a good place to start.</p>
        </div>
    )
}